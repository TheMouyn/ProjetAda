with ada.text_io, ada.integer_text_io, declaration, Outils, Gestion_Dates, Gestion_Sites, Gestion_Personnel, nt_console;
USE Ada.Text_Io, Ada.Integer_Text_Io, Outils, Gestion_Dates, Gestion_Sites, Gestion_Personnel, nt_console;
...
begin -- receptionAMM
  ...
  if regMedicament(nuMedicament).libre = false then
    ...
    put("Numero medicament incorrect, recommencer SVP"); new_line;
  end if;
...
begin -- affichageProduitEnProdSurSite
  ...
  if regSite(choixNuSite).prod = true and regSite(choixNuSite).libre = false then
    put("Les medicaments en production pour ce site sont :"); new_line;
...
procedure affichageProduitGereParResponable(regMedicament : in T_registreMedicament; regPersonnel : in T_registrePersonnel; regSite : in T_registreSite) is
  -- affiche les produits gere pas un responsable de recherche
...
procedure AffichageProduitEnProdSurVille(regMedicament : in T_registreMedicament; regPersonnel : in T_registrePersonnel; regSite : in T_registreSite) is
  -- Affichage cible de la liste des produits en production  dans une ville donnee
...
procedure affichageMedicamentCategorie(regMedicament : in T_registreMedicament; regPersonnel : in T_registrePersonnel; regSite : in T_registreSite) is
  -- Affichage cible de la liste des medicaments d'une categorie donnee (avec les informations sur les sites de production)
  choixCategorie : T_categorie;

begin -- affichageMedicamentCategorie
  saisieCategorie(choixCategorie);
  new_line;
  put_line("Liste des medicaments qui appartiennent a cette categorie : ");
...
begin -- miseEnProduction
  loop -- boucle generale qui permet la confirmation et de recommencer
    loop -- saisie controlee d'un numero de medicament dans le registre avec demande de visualisation du registre medicament
      if choixQuitter then
        exit;
      end if;

      put_line("Quel est le numero du medicament a mettre en production ?");
      put_line("Voulez-vous voir le registre des medicaments ?");
      saisieBoolean(choixBool);
      if choixBool then
        VisualtisationMedicament(regMedicament, regPersonnel, regSite);
        new_line;
      ...
      if regMedicament(choixMedicament).libre = false then
        if nbSiteProductionMedicament(choixMedicament, regMedicament) < MaxSitesProd then
          if regMedicament(choixMedicament).AMM then
            ...
            put_line("Le medicament n'a pas recu d'AMM, il ne peut pas etre mis en production");
          ...
          choixQuitter:=desirQuitter; -- demande si l'utilisateur veut quitter la procedure actuelle
        end if;
      else
        put_line("Le numero saisi ne correspond pas a un medicament du registre");
        choixQuitter:=desirQuitter; -- demande si l'utilisateur veut quitter la procedure actuelle
      end if;
    end loop;
    ...
    loop -- saisie controlee du chef de prod avec visualisation du regPersonnel si besoin
      if choixQuitter then
        exit;
      end if;
    ...
    if choixBool then
      -- toutes les conditions sont validees -> association des variables
...
-- un arret de production d'un medicament donne sur un site donne
choixBool : boolean;
choixMedicament, choixSite, chefProdSiteEnCours : integer;
choixQuitter : boolean:=false;
verifChefProf : boolean := false; -- vrai s'il y a bien un chef de prod associe au site saisi
...
begin -- arretDeProduction
  loop -- boucle generale qui permet la confirmation et de recommencer
    ...
    if choixBool then
      ...
      --verification s'il n'y a plus de chef de prod -> le medicament n'est plus en production
      for i in regMedicament(choixMedicament).chefProd'range loop
      ...
      put_line("Arret de  production annule");
      put_line("Voulez-vous recommencer ?");
      saisieBoolean(choixBool);
      if choixBool = false then
        exit;
...
begin -- nouveauMedicament
  ...
  if registreLibre then
    loop -- boucle de confirmation
      ...
      -- gesiton du nom medicament, verifie s'il n'y a pas un medicament qui a le meme nom et donc verifier la categorie
      ...
      -- saisie de la categorie si nouveau medicament si non on reprend la categorie de l'autre medicament deja present dans le registre
      -- Saisie du type de patient pour ce medicament ou automatique si l'autre est deja present
      ...
      if existeDejaAdulte or existeDejaPediatrique then
        for i in regMedicament'range loop
          if regMedicament(i).nom = nomMed then
            ...
            if regMedicament(i).typePatient = adulte then -- copie du type patient sur nouveau medicament
              typePatientMed := pediatrique;
            else
              typePatientMed := adulte;
            end if;

            --verifier que le resp peut encore etre a la charge d'un produit en plus
            if regPersonnel(regMedicament(i).respRecherche).nbProduit = MaxProdCh then
              ...
              respRetDMed := regMedicament(i).respRecherche; -- copie du numero employe du resp recherche de l'autre medicament
            end if;
      ...
      else -- si nouveau medicament non connu dans le registre
        ...
        loop -- saisie d'un responsable de recherche
          ...
          put_line("Quel est le numero du responsable de recherche de ce medicament ?");
          put_line("Voulez-vous voir le registre des personnels ? ");
          saisieBoolean(choixBool);
          if choixBool then
            VisualisationPersonnel(regPersonnel, regSite);
      ...
      -- Confirmation
      put("Vous voulez ajouter un medicament "); afficherTexte(nomMed); put(" de categorie "); put(T_categorie'image(categorieMed)); new_line;
      ...
      put_line("Le medicament que vous ajoutez est en phase de recherche");
      ...
      if choixBool then
        -- toutes les conditions sont validees -> association des variables dans le registre
        for i in regMedicament'range loop
          if regMedicament(i).libre then
    ...
    put_line("Registre medicament sature, ajout impossible");
  end if;
...
begin -- supressionMedicament
  -- choix de medicament
    ...
    -- verifier s'il est en prod (ce med est encore en prod sur les sites : )-> arret prod automatique ?
  loop -- boucle de confirmation
    loop --boucle saisie medicament
    ...
    end loop; --boucle saisie medicament

    if choixQuitter then
      exit;
    end if;

    -- si le medicament est toujours en production
    if estEnProd then
      put_line("Ce medicament est encore en production, voulez-vous arreter le(s) chaine(s) de production automatiquement?");
      saisieBoolean(choixBool);
      if choixBool then
        choixArretProd := true;
    ...
    if estEnProd and choixArretProd then
      put("Qui est en production sur le(s) site(s) : ");
      for i in regMedicament(choixMedicament).chefProd'range loop
        if regMedicament(choixMedicament).chefProd(i).libre = false then
